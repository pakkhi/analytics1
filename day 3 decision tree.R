...
- performs only binary split for r part 
...
- generates trees which ar ecalled CHAID(Chi-square automatic interaction detector) chaid is unsupervised learning while rpart is supervised
...
library(rpart.plot)
library(caret)# one shop for machine learning 
library(randomForest)#decisions from multiple trees combined as average, all trees have different data randomly picked from the main data
```
...
table(data$survived)#TO FIND the frequency of those survived
...
```{r}
set.seed(123)
fit <- rpart(survived ~ ., data = train, method = 'class',control = rpart.control(cp = .0001))# . after ~ gives all the x varibles, we are predicting a categorical variable so method= class...for decision tree nova is given  
fit#we chose cp as randomly here to generate the complicated chart
rpart.plot(fit,cex=.8,nn=T, extra=104)#cex shows size
printcp(fit)
test$pred <- predict(fit, test, type = "class")
table(test$pred,test$survived)#we are actually creating a confusion matrix where we are saying prediction and actual value says 135 people dont survive while 68 both says survived , 26 and 32 are error or variations
...
```{r}
prunetree2 = prune(fit, cp=0.005)#we chose cp=.005 as it is showing least errorin table command
printcp(prunetree2)
...
```{r}
fit <- randomForest(survived ~ ., data = train,ntree=500,na.action = na.omit) #we are making a code for making forest...where ntree is no of trees to be made r 500 and na action is for values which are empty just omit them
