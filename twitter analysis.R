...
library("curl")
library("twitteR")#twitter analysis package
library("ROAuth")
library("syuzhet") #library for sentiment analysis - comparison

#key to connect to twitter
...
tweets.df2 <- gsub("http.*","",tweets_user$text)#gsub is a replacement function for replacing something with other thing
...
tweets[1:5]
n.tweet <- length(tweets)#length of tweets giv no. of tweets
n.tweet
...
emotion.df2
word.df
#use engrams - combining words, expand dictionary -----to make analysis better
#-----
sent.value <- get_sentiment(word.df)#getting us sentiment out of those things 
sent.value
tweets[c(16,62)]
