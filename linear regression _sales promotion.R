...
omni = omni1
head(omni)
str(omni) #what types of data 
nrow(omni)
dim(omni)#how many rows and columns
#MLR  Create Multiple Linear Regression
# we want to see how Sales Qty depend on Price and Promotion Values
fit2 = lm(sales ~ price + promotion, data=omni) #two variables so we separated them through + sign
...
#keeping promotion constant, if price is increased by 1 unit, salesqty decreases by 3 units
...
fitted(fit2) #predicted sales values for input/actual price and promotion
omni$sales #actual sales
residuals(fit2) #diff btw predicted and actual
...
#Assumptions 
par(mfrow=c(2,2))#par= multiple frames per row (2rows and 2 columns)
plot(fit2)
par(mfrow=c(1,1))

plot(fit2,which=1)  # no pattern, equal variance --'which' is not necessary to write
plot(fit2,2)  # Residuals are normally distributed
plot(fit2,3)  # No hetero-scedascity - we had a homosedacity that is points are being equally scattered
plot(fit2,4)  # tells outliers which affect model - outliers are one which is quite unrelated to data like all class got 60 marks while 3 people got 100 and 0 0 
omni[c(11,14,15),]#we removed the outliers
