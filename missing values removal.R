...
```
for (i in titanic) {sum(is.na(i))
  
}
```{r}
sapply(titanic,function(x) sum(is.na(x)))#apply is a substitute for 'for' loop better to use apply family
mean(titanic$age)
...
```{r}
impute(titanic$age,mean)#imputing the mean value in place of missing value as when we use logistics it will give error 
impute(titanic$age,median)#we use median when data has a lot of variations 
...
4. **KNN imputation**
  ...
  benefits of knnimputation is we dont have to define values for each colomn
```{r}
library(DMwR)
knnOutput <- knnImputation(titanic[, !names(titanic) %in% "survived"])#i am not using y variable that is !names...  
anyNA(knnOutput)
...
#anyNA(miceOutput)
```
#how logistic regression creates dummy variable when given with categorical variables 
str(titanic)

g1 <-glm(survived ~ age + sex, data = titanic, family = 'binomial')
g1    
summary(g1)#gives sex male as a new row as it alphabetically chose female as base and chose male as category to interpret this ...1 unit increase in age cause -0.004 survival rate
